...
public class ParentDao {
	...
	public List<Parent> getParentByTelePhone_Name(String tele_name) {
		...
		try {
			conn = DBConnection.getConnection();
			String sql = "select studentNumber,parentId,parentName,parentNickName,parentHeader,parentPwd,parentSex,parentTelePhone,address,backgroundPhoto from parent where parentTelePhone = ? or parentName like ?";
			pStatement = conn.prepareStatement(sql);
			
			pStatement.setString(1, tele_name);
			pStatement.setString(2, "%" + tele_name + "%");
			rSet = pStatement.executeQuery();
			Parent p = null;
			list = new ArrayList<>();
			Student s = null;
