...
public class StorySongDao 
{
	...
	//查找某一个故事儿歌表，按儿歌名称查询,有则返回对象，没有返回null,成员变量只要给外键属性即可
	public StorySong getStorySongByName(String name)
	{
		Connection conn=null;
		PreparedStatement pStatement=null;
		ResultSet rSet =null;
		...
		try {
			conn=DBConnection.getConnection();
			String sql ="select ssId,ssType,ssName,ssThumbnail,ssURL,ssBrief from storysong where ssName=?";
			pStatement =conn.prepareStatement(sql);
	...
	//返回所有故事或者儿歌的对象,按类型分对象
		...
		//实现故事儿歌表的分页查询，按类型分类
		public List<StorySong> getPageStorySong (int curPage,int pageSize,int type)
		{
			Connection conn=null;
			PreparedStatement pStatement=null;
			ResultSet rSet =null;
			...
			try {
				conn=DBConnection.getConnection();
				int firstRecoder = (curPage-1)*pageSize;
				String sql ="select ssId,ssType,ssName,ssThumbnail,ssURL,ssBrief from storysong where ssType=? limit ?,?";
				pStatement =conn.prepareStatement(sql);
				pStatement.setInt(1, type);
